...
import { categoryItem } from "../assets/assets.js"
        ...
        <ul className='flex flex-wrap justify-center gap-4'>
          {
            categoryItem.map((item, index)=>(
              ...
              className={ `cursor-pointer px-6 py-2 rounded-full font-medium text-sm transition-all duration-200 ${category === item.category_title ? "bg-amber-500": "bg-gray-200 hover:bg-amber-100"}`}>
                {item.category_title}
              </li>
            ))
          }
...
<div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
  {
    products.length > 0 ? (
      ...
      <p className='text-sm col-span-2 text-gray-500'>
